...
static void pre_write_log(char *filename, long fileline, const log_print_type_e type, const char* format, const va_list varArgs)
{
	...
	if (win_time.wHour == 0 && g_log_handle->last_hour == 23){
		open_new_log();
	}
	g_log_handle->last_hour = win_time.wHour;
#else
	struct timeval tv;
	...
	if (ptm->tm_hour == 0 && g_log_handle->last_hour == 23){
		open_new_log();
	}
	g_log_handle->last_hour = ptm->tm_hour;
#endif
